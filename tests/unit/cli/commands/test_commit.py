...
class TestCommitCommand:
    ...
    def test_commit_command_creates_new_commit(
        self,
        mock_get_repo: Mock,
        mock_preview: Mock,
        mock_result: Mock,
    ...
    @patch("spec_cli.cli.commands.commit.get_spec_repository")
    def test_commit_command_auto_stages_changes(
        self, mock_get_repo: Mock, mock_auto_stage: Mock
    ) -> None:
        """Test commit command auto-stages changes with --all flag."""
        # Mock repository
        mock_repo = Mock()
        mock_repo.get_git_status.side_effect = [
            {"staged": [], "modified": ["test.py"], "untracked": []},
            {"staged": ["test.py"], "modified": [], "untracked": []},
        ]
        mock_repo.commit.return_value = "abc123def456"
        mock_get_repo.return_value = mock_repo
        ...
        with patch(
            ...
            result = self.runner.invoke(
                commit_command, ["-a", "-m", "Auto stage commit"]
            )
    ...
    @patch("spec_cli.cli.commands.commit._show_commit_preview")
    @patch("spec_cli.cli.commands.commit.get_spec_repository")
    def test_commit_command_amends_last_commit(
        self, mock_get_repo: Mock, mock_preview: Mock, mock_result: Mock
    ) -> None:
    ...
    @patch("spec_cli.cli.commands.commit.get_spec_repository")
    def test_commit_command_dry_run_preview(
        self, mock_get_repo: Mock, mock_show_message: Mock, mock_preview: Mock
    ) -> None:
        ...
        result = self.runner.invoke(
            commit_command, ["--dry-run", "-m", "Dry run commit"]
        )
    ...
    @patch("spec_cli.cli.commands.commit.show_message")
    @patch("spec_cli.cli.commands.commit.get_spec_repository")
    def test_commit_command_no_staged_changes(
        self, mock_get_repo: Mock, mock_show_message: Mock
    ) -> None:
    ...
    @patch("spec_cli.cli.commands.commit.show_message")
    @patch("spec_cli.cli.commands.commit.get_spec_repository")
    def test_commit_command_no_changes_clean_directory(
        self, mock_get_repo: Mock, mock_show_message: Mock
    ) -> None:
    ...
    @patch("spec_cli.cli.commands.commit.show_message")
    @patch("spec_cli.cli.commands.commit.get_spec_repository")
    def test_commit_command_user_cancels(
        self, mock_get_repo: Mock, mock_show_message: Mock, mock_confirm: Mock
    ) -> None:
    ...
    @patch("spec_cli.cli.commands.commit.get_spec_repository")
    def test_commit_command_repository_error(self, mock_get_repo: Mock) -> None:
    ...
    def test_commit_command_help(self) -> None:
...
class TestCommitUtilityFunctions:
    ...
    @patch("spec_cli.cli.commands.commit.show_message")
    def test_auto_stage_changes_success(self, mock_show_message: Mock) -> None:
        """Test auto-staging changes successfully."""
        mock_repo = Mock()
        status = {"modified": ["test1.py", "test2.py"], "deleted": ["old.py"]}

        _auto_stage_changes(mock_repo, status)

        # Should stage modified files using add_files (plural)
        assert mock_repo.add_files.call_count == 2
        mock_repo.add_files.assert_any_call(["test1.py"])
        mock_repo.add_files.assert_any_call(["test2.py"])

        # Deleted files are not implemented yet (placeholder code)
        mock_repo.remove_file.assert_not_called()

        # Total count includes deleted files even though they're not implemented
        mock_show_message.assert_called_with("Auto-staged 3 files", "info")

    @patch("spec_cli.cli.commands.commit.show_message")
    def test_auto_stage_changes_with_errors(self, mock_show_message: Mock) -> None:
        """Test auto-staging changes with some errors."""
        mock_repo = Mock()
        mock_repo.add_files.side_effect = [None, Exception("Add error")]
        status = {"modified": ["test1.py", "test2.py"], "deleted": []}

        _auto_stage_changes(mock_repo, status)

        # Should attempt to stage both files using add_files
        assert mock_repo.add_files.call_count == 2
        mock_show_message.assert_called_with("Auto-staged 2 files", "info")

    @patch("spec_cli.cli.commands.commit.get_console")
    def test_show_commit_preview_few_files(self, mock_get_console: Mock) -> None:
    ...
    @patch("spec_cli.cli.commands.commit.get_console")
    def test_show_commit_preview_many_files(self, mock_get_console: Mock) -> None:
    ...
    @patch("spec_cli.cli.commands.commit.get_console")
    def test_show_commit_preview_amend(self, mock_get_console: Mock) -> None:
    ...
    def test_show_commit_result_success(
        self, mock_status_table: Mock, mock_get_console: Mock
    ) -> None:
        ...
        mock_table.add_status_item.assert_called()
        mock_table.print.assert_called_once()

    @patch("spec_cli.cli.commands.commit.get_console")
    def test_show_commit_result_get_info_error(self, mock_get_console: Mock) -> None:
    ...
    def test_commit_command_message_required(self) -> None:
