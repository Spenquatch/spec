...
class TestInitCommand:
    ...
    def test_init_command_success(self, mock_git_repo: MagicMock) -> None:
    ...
    @patch("spec_cli.cli.commands.init.SpecGitRepository")
    def test_init_command_already_initialized(self, mock_git_repo: MagicMock) -> None:
    ...
    @patch("spec_cli.cli.commands.init.SpecGitRepository")
    def test_init_command_force_reinitialize(self, mock_git_repo: MagicMock) -> None:
    ...
    @patch("spec_cli.cli.commands.init.SpecGitRepository")
    def test_init_command_repository_error(self, mock_git_repo: MagicMock) -> None:
    ...
    @patch("spec_cli.cli.commands.init.SpecGitRepository")
    def test_init_command_initialization_verification_fails(
        self, mock_git_repo: MagicMock
    ) -> None:
        """Test init command when initialization verification fails."""
        mock_repo = MagicMock()
        mock_repo.is_initialized.side_effect = [
            False,
            False,
        ]  # Not initialized before or after
        mock_repo.initialize.return_value = None
        mock_git_repo.return_value = mock_repo
    ...
    @patch("spec_cli.cli.commands.init.SpecGitRepository")
    def test_init_command_unexpected_error(self, mock_git_repo: MagicMock) -> None:
