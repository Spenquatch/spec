...
class TestFileCacheEntry:
    ...
    def test_cache_entry_creation_and_serialization(self):
        ...
        expected_keys = {
            "file_path", "hash_md5", "hash_sha256", "size", 
            "mtime", "last_processed", "metadata"
        }
...
class TestFileCacheManager:
    ...
    def test_cache_manager_load_save_cycle(self, temp_cache_manager):
    ...
    def test_cache_manager_entry_crud_operations(self, temp_cache_manager):
    ...
    def test_cache_cleanup_stale_entries(self, temp_cache_manager):
        ...
        removed_count = manager.cleanup_stale_entries(set(), max_age_days=5)  # No existing files
        assert removed_count == 1  # recent_file.py removed (not in existing_files)
        assert len(manager.get_all_entries()) == 0
    
    def test_cache_statistics_and_validation(self, temp_cache_manager):
    ...
    def test_cache_export_functionality(self, temp_cache_manager):
    ...
    def test_cache_clear_functionality(self, temp_cache_manager):
        """Test cache clearing."""
        manager = temp_cache_manager
    ...
    def test_cache_file_error_handling(self, temp_cache_manager):
