...
class TestProgressEvent:
    ...
    def test_progress_event_creation_and_serialization(self):
        ...
        expected_keys = {
            "event_type", "timestamp", "file_path", "stage",
            "progress", "total_files", "processed_files", "message", "metadata"
        }
    ...
    def test_progress_event_with_minimal_data(self):
...
class TestProgressReporter:
    ...
    def test_progress_reporter_listener_management(self, progress_reporter):
    ...
    def test_progress_reporter_event_emission(self, progress_reporter):
    ...
    def test_progress_reporter_event_storage_and_retrieval(self, progress_reporter):
        """Test event storage and retrieval functionality."""
        # Create multiple events
        events = [
            ProgressEvent(event_type=ProgressEventType.FILE_STARTED, message=f"Event {i}")
            for i in range(15)
        ]
        ...
        # Test events by type
        file_started_events = progress_reporter.get_events_by_type(ProgressEventType.FILE_STARTED)
        ...
        for i in range(progress_reporter.max_events):
            progress_reporter.emit_event(
                ProgressEvent(event_type=ProgressEventType.WARNING, message=f"Extra {i}")
            )
        
        # Should not exceed max_events
        assert len(progress_reporter.events) <= progress_reporter.max_events
    
    def test_progress_reporter_convenience_methods(self, progress_reporter):
        ...
        assert "Starting batch" in events[0].message
        ...
        # Test stage update
        progress_reporter.emit_stage_update(file_path, ProcessingStage.CONFLICT_DETECTION, "Checking conflicts")
        ...
        progress_reporter.emit_conflict_detected(file_path, "content_modified", "merge_intelligent")
        events = progress_reporter.get_events_by_type(ProgressEventType.CONFLICT_DETECTED)
        assert len(events) == 1
    ...
    def test_progress_reporter_summary_generation(self, progress_reporter):
    ...
    def test_progress_reporter_clear_events(self, progress_reporter):
...
class TestProgressEventTypes:
    ...
    def test_progress_event_type_enumeration(self):
        """Test all progress event types are defined."""
        expected_types = {
            "batch_started", "batch_completed", "batch_failed",
            "file_started", "file_completed", "file_failed", "file_skipped",
            "stage_started", "stage_completed",
            "conflict_detected", "conflict_resolved",
            "progress_update", "warning", "error"
        }
        
        actual_types = {event_type.value for event_type in ProgressEventType}
        assert actual_types == expected_types
    
    def test_processing_stage_enumeration(self):
        """Test all processing stages are defined."""
        expected_stages = {
            "initialization", "change_detection", "content_generation",
            "conflict_detection", "conflict_resolution", "file_writing",
            "cache_update", "cleanup"
        }
...
class TestGlobalProgressReporter:
    ...
    def test_global_progress_reporter_exists(self):
    ...
    def test_event_message_generation(self):
        ...
        reporter.emit_stage_update(file_path, ProcessingStage.CONTENT_GENERATION, "Custom message")
        events = reporter.get_events_by_type(ProgressEventType.STAGE_STARTED)
