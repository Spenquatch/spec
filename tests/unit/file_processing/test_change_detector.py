...
class TestFileChangeDetector:
    ...
    def change_detector(self, mock_settings: MagicMock) -> FileChangeDetector:
        ...
        with (
            ...
            detector.cache_manager.get_cache_entry = MagicMock()
            detector.cache_manager.update_cache = MagicMock()
            detector.metadata_extractor.get_file_metadata = MagicMock()
            return detector

    def test_file_hash_calculation_md5_sha256(
    ...
    def test_file_hash_calculation_large_files(
        self, change_detector: FileChangeDetector
    ) -> None:
    ...
    def test_file_hash_calculation_error_handling(
        self, change_detector: FileChangeDetector
    ) -> None:
    ...
    def test_get_file_info(self, change_detector: FileChangeDetector) -> None:
        ...
        try:
            ...
            cast(
                MagicMock, change_detector.metadata_extractor.get_file_metadata
            ).return_value = {
                "language": "python",
                "type": "script",
            }
    ...
    def test_file_change_detection_stats_based(
        self, change_detector: FileChangeDetector
    ) -> None:
        ...
        try:
            ...
            cast(MagicMock, change_detector.cache_manager.get_entry).return_value = None
            ...
            cast(
                MagicMock, change_detector.cache_manager.get_entry
    ...
    def test_file_change_detection_hash_based(
        self, change_detector: FileChangeDetector
    ) -> None:
    ...
    def test_file_change_detection_new_files(
        self, change_detector: FileChangeDetector
    ) -> None:
        ...
        cast(MagicMock, change_detector.cache_manager.remove_entry).assert_called_with(
            str(non_existent)
        )

    def test_file_change_detection_deleted_files(
        self, change_detector: FileChangeDetector
    ) -> None:
    ...
    def test_update_file_cache(self, change_detector: FileChangeDetector) -> None:
        ...
        try:
            ...
            cast(
                MagicMock, change_detector.cache_manager.set_entry
            ).assert_called_once()
    ...
    def test_directory_change_detection(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            cast(
                MagicMock, change_detector.ignore_matcher.should_ignore
            ).return_value = False

            # Mock cache manager responses
            cast(
                MagicMock, change_detector.cache_manager.get_entry
            ).return_value = None  # All files are new
            cast(
                MagicMock, change_detector.cache_manager.get_all_entries
            ).return_value = {}
    ...
    def test_files_needing_processing_determination(
        self, change_detector: FileChangeDetector
    ) -> None:
        ...
        # Test force_all=True
        result = change_detector.get_files_needing_processing(
            file_paths, force_all=True
        )
        assert result == file_paths

        # Test selective processing
        def mock_has_changed(path: Path) -> bool:
            return str(path).endswith("file1.py") or str(path).endswith("file3.py")

        with patch.object(
            change_detector, "has_file_changed", side_effect=mock_has_changed
        ):
            result = change_detector.get_files_needing_processing(
                file_paths, force_all=False
            )
    ...
    def test_change_detection_with_ignore_patterns(
        self, change_detector: FileChangeDetector
    ) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Mock ignore matcher
            def mock_should_ignore(path: Path) -> bool:
                return str(path).endswith(".pyc")

            cast(
                MagicMock, change_detector.ignore_matcher.should_ignore
            ).side_effect = mock_should_ignore
            cast(MagicMock, change_detector.cache_manager.get_entry).return_value = None
            cast(
                MagicMock, change_detector.cache_manager.get_all_entries
            ).return_value = {}
    ...
    def test_cache_cleanup(self, change_detector: FileChangeDetector) -> None:
        ...
        with (
            ...
            cast(
                MagicMock, change_detector.cache_manager.cleanup_stale_entries
            ).return_value = 5
            # save_cache method is already mocked via the mock cache_manager
            ...
            cast(
                MagicMock, change_detector.cache_manager.save_cache
            ).assert_called_once()

    def test_error_handling_in_change_detection(
        self, change_detector: FileChangeDetector
    ) -> None:
    ...
    def test_save_cache_delegation(self, change_detector: FileChangeDetector) -> None:
        """Test that save_cache delegates to cache manager."""
        change_detector.save_cache()
        cast(MagicMock, change_detector.cache_manager.save_cache).assert_called_once()
