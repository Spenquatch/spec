...
class TestBatchProcessingOptions:
    ...
    def test_batch_processing_options_defaults(self):
    ...
    def test_batch_processing_options_custom_values(self):
...
class TestBatchProcessingResult:
    ...
    def test_batch_processing_result_initialization(self):
    ...
    def test_batch_processing_result_duration_calculation(self):
    ...
    def test_batch_processing_result_serialization(self):
        ...
        expected_keys = {
            "success", "total_files", "successful_count", "failed_count",
            "skipped_count", "successful_files", "failed_files", "skipped_files",
            "errors", "warnings", "duration", "workflow_id"
        }
...
class TestBatchFileProcessor:
    ...
    @pytest.fixture
    def mock_processor(self, mock_settings):
        """Create BatchFileProcessor with mocked dependencies."""
        with patch('spec_cli.file_processing.batch_processor.FileChangeDetector'), \
    ...
    def test_batch_processor_sequential_processing(self, mock_processor):
        """Test sequential processing of files."""
        file_paths = [Path(f"file{i}.py") for i in range(3)]
        options = BatchProcessingOptions()
        
        # Mock change detector
        mock_processor.change_detector.get_files_needing_processing.return_value = file_paths
    ...
    def test_batch_processor_progress_tracking(self, mock_processor):
        ...
        # Process files
        result = mock_processor.process_files(file_paths, options, progress_callback)
    ...
    def test_batch_processor_conflict_handling(self, mock_processor):
        """Test handling of conflicts during batch processing."""
        file_paths = [Path("file1.py"), Path("file2.py")]
        options = BatchProcessingOptions(conflict_strategy=ConflictResolutionStrategy.KEEP_OURS)
    ...
    def test_batch_processor_error_recovery(self, mock_processor):
    ...
    def test_batch_processor_auto_commit_integration(self, mock_processor):
    ...
    def test_batch_processing_options_configuration(self, mock_processor):
        ...
        mock_processor.change_detector.get_files_needing_processing.return_value = file_paths[:3]
        mock_processor.pipeline.process_file.return_value = FileProcessingResult(
            file_path=file_paths[0], success=True
        )
    ...
    def test_batch_processing_validation(self, mock_processor):
        ...
        options = BatchProcessingOptions(conflict_strategy=ConflictResolutionStrategy.MERGE_INTELLIGENT)
    ...
    def test_batch_processing_estimation(self, mock_processor):
        ...
        mock_processor.pipeline.get_processing_estimate.assert_called_once_with(file_paths)
    
    def test_batch_processing_summary_generation(self, mock_processor):
    ...
    def test_batch_processor_skip_unchanged_files(self, mock_processor):
        ...
        # Mock change detector
        mock_processor.change_detector.get_files_needing_processing.return_value = files_needing_processing
        mock_processor.pipeline.process_file.return_value = FileProcessingResult(
    ...
    def test_batch_processor_exception_handling(self, mock_processor):
        """Test handling of unexpected exceptions."""
        file_paths = [Path("file1.py")]
        
        # Mock pipeline to raise exception
        mock_processor.change_detector.get_files_needing_processing.side_effect = Exception("Unexpected error")
...
class TestConvenienceFunctions:
    ...
    def test_process_files_batch_function(self):
        """Test process_files_batch convenience function."""
        file_paths = [Path("test.py")]
        
        with patch('spec_cli.file_processing.batch_processor.BatchFileProcessor') as mock_processor_class:
    ...
    def test_estimate_processing_time_function(self):
