...
class TestFileProcessingResult:
    ...
    def test_processing_result_creation_and_serialization(self) -> None:
    ...
    def test_processing_result_without_conflicts(self) -> None:
        """Test processing result without conflicts."""
        result = FileProcessingResult(file_path=Path("/test/file.py"), success=True)
...
class TestFileProcessingPipeline:
    ...
    def test_file_processing_pipeline_complete_flow(
        ...
        stage_calls = [
            call[0][1] for call in progress_reporter.emit_stage_update.call_args_list
        ]
    ...
    def test_pipeline_conflict_resolution_integration(
        ...
        assert result.conflict_info == conflict_info
        assert (
            result.resolution_strategy == ConflictResolutionStrategy.MERGE_INTELLIGENT
        )
    ...
    def test_pipeline_error_handling(
        ...
        content_generator.generate_spec_content.side_effect = Exception(
            "Generation failed"
        )
    ...
    def test_pipeline_large_file_handling(
    ...
    def test_pipeline_with_custom_conflict_strategy(
        ...
        change_detector.has_file_changed.return_value = True
        content_generator.generate_spec_content.return_value = {
            "index": Path("/specs/file/index.md")
        }
