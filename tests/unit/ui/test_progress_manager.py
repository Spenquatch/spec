...
class TestProgressState:
    ...
    def test_progress_state_with_optional_fields(self) -> None:
    ...
    def test_progress_percentage_normal(self) -> None:
        """Test progress percentage calculation."""
        state = ProgressState(operation_id="test", total_items=10, completed_items=3)
    ...
    def test_progress_percentage_zero_total(self) -> None:
        ...
        state = ProgressState(operation_id="test", total_items=0, completed_items=0)

        assert state.progress_percentage == 0.0

    def test_progress_percentage_complete(self) -> None:
        """Test progress percentage when complete."""
        state = ProgressState(operation_id="test", total_items=5, completed_items=5)

        assert state.progress_percentage == 1.0

    def test_elapsed_time_no_start(self) -> None:
        """Test elapsed time when no start time set."""
        state = ProgressState(operation_id="test", total_items=5, completed_items=2)

        assert state.elapsed_time is None

    @patch("time.time")
    def test_elapsed_time_with_start(self, mock_time: Mock) -> None:
        ...
        state = ProgressState(
            operation_id="test", total_items=5, completed_items=2, start_time=start_time
        )
...
class TestProgressManager:
    ...
    def test_progress_manager_initialization_defaults(
        self, mock_reporter: Mock, mock_spinner_manager: Mock, mock_progress_bar: Mock
    ) -> None:
    ...
    @patch("spec_cli.ui.progress_manager.SpecProgressBar")
    @patch("spec_cli.ui.progress_manager.SpinnerManager")
    def test_progress_manager_initialization_custom(
        self, mock_spinner_manager: Mock, mock_progress_bar: Mock
    ) -> None:
        ...
        manager = ProgressManager(
            progress_reporter_instance=custom_reporter, auto_display=auto_display
        )
    ...
    @patch("spec_cli.ui.progress_manager.SpinnerManager")
    @patch("spec_cli.ui.progress_manager.progress_reporter")
    def test_handle_progress_event(
        self, mock_reporter: Mock, mock_spinner_manager: Mock, mock_progress_bar: Mock
    ) -> None:
        ...
        # Create test event
        event = ProgressEvent(
            event_type=ProgressEventType.BATCH_STARTED, message="Test batch started"
        )
    ...
    @patch("spec_cli.ui.progress_manager.SpinnerManager")
    @patch("spec_cli.ui.progress_manager.progress_reporter")
    def test_handle_progress_event_handler_exception(
        self, mock_reporter: Mock, mock_spinner_manager: Mock, mock_progress_bar: Mock
    ) -> None:
    ...
    @patch("spec_cli.ui.progress_manager.progress_reporter")
    @patch("time.time")
    def test_handle_batch_started(
        self,
        mock_time: Mock,
        mock_reporter: Mock,
        mock_spinner_manager: Mock,
    ...
    def test_handle_batch_completed(
        self,
    ...
    def test_handle_batch_failed(
        ...
        event = ProgressEvent(
            event_type=ProgressEventType.BATCH_FAILED, message="Batch failed"
        )

        manager._handle_batch_failed(event)

        # Check error message
        mock_console.print_status.assert_called_once_with("Batch failed", "error")

    @patch("spec_cli.ui.progress_manager.SpecProgressBar")
    @patch("spec_cli.ui.progress_manager.SpinnerManager")
    @patch("spec_cli.ui.progress_manager.progress_reporter")
    def test_handle_file_started(
        self, mock_reporter: Mock, mock_spinner_manager: Mock, mock_progress_bar: Mock
    ) -> None:
        ...
        operation_id = "test_batch_1000"
        state = ProgressState(
            operation_id=operation_id, total_items=5, completed_items=2
        )
        manager.progress_states[operation_id] = state
        manager.active_operations[operation_id] = "progress_bar:task1"
    ...
    @patch("spec_cli.ui.progress_manager.SpinnerManager")
    @patch("spec_cli.ui.progress_manager.progress_reporter")
    def test_handle_file_completed(
        self, mock_reporter: Mock, mock_spinner_manager: Mock, mock_progress_bar: Mock
    ) -> None:
    ...
    def test_handle_stage_started(
        ...
        with patch.object(manager, "_update_operation_text") as mock_update_text:
            ...
            mock_update_text.assert_called_once_with(
                operation_id, "Stage: content_generation"
            )

    @patch("spec_cli.ui.progress_manager.SpecProgressBar")
    @patch("spec_cli.ui.progress_manager.SpinnerManager")
    @patch("spec_cli.ui.progress_manager.progress_reporter")
    def test_update_progress_display_progress_bar(
        self, mock_reporter: Mock, mock_spinner_manager: Mock, mock_progress_bar: Mock
    ) -> None:
        ...
        event = ProgressEvent(
            event_type=ProgressEventType.PROGRESS_UPDATE, message="Processing"
        )
        ...
        mock_progress_bar_instance.update_task.assert_called_once_with(
            "task1", completed=3, description="Processing: file.txt"
        )

    @patch("spec_cli.ui.progress_manager.SpecProgressBar")
    @patch("spec_cli.ui.progress_manager.SpinnerManager")
    @patch("spec_cli.ui.progress_manager.progress_reporter")
    def test_update_progress_display_spinner(
        self, mock_reporter: Mock, mock_spinner_manager: Mock, mock_progress_bar: Mock
    ) -> None:
    ...
    @patch("spec_cli.ui.progress_manager.SpinnerManager")
    @patch("spec_cli.ui.progress_manager.progress_reporter")
    def test_find_active_operation(
        self, mock_reporter: Mock, mock_spinner_manager: Mock, mock_progress_bar: Mock
    ) -> None:
    ...
    @patch("spec_cli.ui.progress_manager.SpinnerManager")
    @patch("spec_cli.ui.progress_manager.progress_reporter")
    def test_cleanup_operation_progress_bar(
        self, mock_reporter: Mock, mock_spinner_manager: Mock, mock_progress_bar: Mock
    ) -> None:
    ...
    @patch("spec_cli.ui.progress_manager.SpinnerManager")
    @patch("spec_cli.ui.progress_manager.progress_reporter")
    def test_cleanup_operation_spinner(
        self, mock_reporter: Mock, mock_spinner_manager: Mock, mock_progress_bar: Mock
    ) -> None:
    ...
    @patch("spec_cli.ui.progress_manager.progress_reporter")
    @patch("time.time")
    def test_start_indeterminate_operation(
        self,
        mock_time: Mock,
        mock_reporter: Mock,
        mock_spinner_manager: Mock,
        ...
        mock_spinner_manager_instance.create_spinner.assert_called_once_with(
            operation_id, message
        )
        mock_spinner_manager_instance.start_spinner.assert_called_once_with(
            operation_id
        )
    ...
    def test_add_event_handler(
        ...
        assert (
            len(manager._event_handlers[ProgressEventType.BATCH_STARTED])
            == initial_count + 1
        )
        assert (
            custom_handler in manager._event_handlers[ProgressEventType.BATCH_STARTED]
        )
    ...
    def test_remove_event_handler(
        ...
        # Remove it
        result = manager.remove_event_handler(
            ProgressEventType.BATCH_STARTED, custom_handler
        )
        assert result is True
        assert (
            custom_handler
            not in manager._event_handlers[ProgressEventType.BATCH_STARTED]
        )
...
class TestGlobalProgressManager:
    ...
    def test_get_progress_manager_creates_instance(
        self, mock_progress_manager_class: Mock
    ) -> None:
    ...
    @patch("spec_cli.ui.progress_manager.ProgressManager")
    def test_get_progress_manager_returns_existing(
        self, mock_progress_manager_class: Mock
    ) -> None:
    ...
    def test_set_progress_manager(self) -> None:
...
class TestProgressManagerIntegration:
    ...
    def test_complete_batch_workflow(
        ...
        with patch("spec_cli.ui.progress_manager.get_console") as mock_get_console:
            ...
            mock_console.print_status.assert_called_with(
    ...
    def test_custom_event_handlers(
        ...
        def custom_batch_handler(event: Any) -> None:
            batch_started_calls.append(event)

        def custom_file_handler(event: Any) -> None:
        ...
        file_event = ProgressEvent(
            event_type=ProgressEventType.FILE_COMPLETED, message="Custom file complete"
        )
    ...
    @patch("spec_cli.ui.progress_manager.progress_reporter")
    @patch("time.time")
    def test_indeterminate_operation_workflow(
        self,
        mock_time: Mock,
        mock_reporter: Mock,
        mock_spinner_manager: Mock,
