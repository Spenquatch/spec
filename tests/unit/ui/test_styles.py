...
class TestSpecStyles:
    ...
    def test_spec_styles_formatting(self) -> None:
    ...
    def test_style_helpers_and_utilities(self) -> None:
        ...
        assert (
            format_status("message", "success", False) == "[success]message[/success]"
        )
...
class TestPathFormatting:
    ...
    def test_path_auto_detection(self) -> None:
        ...
        for path in spec_paths:
            result = format_path(str(path), "auto")
            assert "[spec_file]" in result

        # Test file detection by extension
        file_paths = ["script.py", "document.txt", "config.json", Path("src/main.cpp")]
        for path in file_paths:
            result = format_path(str(path), "auto")
            assert "[file]" in result

        # Test directory detection (no extension)
        dir_paths = ["src", "tests", "documentation", Path("build")]
        for path in dir_paths:
            result = format_path(str(path), "auto")
            assert "[directory]" in result

    def test_path_formatting_edge_cases(self) -> None:
...
class TestStyleIntegration:
    ...
    def test_style_integration_with_themes(self) -> None:
        """Test that styles work correctly with different themes."""
        # Test that create_rich_text handles missing styles gracefully
        with patch("spec_cli.ui.styles.get_current_theme") as mock_theme:
            mock_theme_obj3: Mock = Mock()
            mock_theme_obj3.get_style.return_value = ""  # Empty style
            mock_theme.return_value = mock_theme_obj3

            text_obj = create_rich_text("text", "nonexistent_style")
            assert isinstance(text_obj, Text)
            assert str(text_obj) == "text"

    def test_format_status_integration(self) -> None:
        ...
        with patch("spec_cli.ui.styles.get_current_theme") as mock_theme:
            mock_theme_obj5: Mock = Mock()
            mock_theme_obj5.get_emoji_replacements.return_value = {
                "✅": "[success]✓[/success]"
                # Missing other emojis
            }
            mock_theme.return_value = mock_theme_obj5
