...
class TestProgressContext:
    ...
    def test_progress_context_determinate(self, mock_simple_progress):
        """Test progress context with determinate progress."""
        mock_progress_bar = Mock()
        mock_simple_progress.return_value.__enter__ = Mock(return_value=mock_progress_bar)
    ...
    def test_progress_context_indeterminate_with_spinner(self, mock_spinner_context):
        ...
        with progress_context(total_items=None, description="Loading", show_spinner=True) as update_func:
            # Test that we get an update function
            assert callable(update_func)
    ...
    def test_progress_context_indeterminate_no_spinner(self):
        """Test progress context with indeterminate progress without spinner."""
        with progress_context(total_items=None, description="Silent", show_spinner=False) as update_func:
            # Test that we get an update function
            assert callable(update_func)
...
class TestTimedOperation:
    ...
    @patch('time.time')
    def test_timed_operation_basic(self, mock_time):
    ...
    @patch('time.time')
    def test_timed_operation_no_logging(self, mock_time):
    ...
    @patch('time.time')
    def test_timed_operation_with_exception(self, mock_time):
...
class TestCreateFileProgressTracker:
    ...
    @patch('time.time')
    def test_create_file_progress_tracker(self, mock_time, mock_get_manager):
    ...
    @patch('spec_cli.ui.progress_utils.get_progress_manager')
    @patch('time.time')
    def test_create_file_progress_tracker_empty_list(self, mock_time, mock_get_manager):
...
class TestProgressTracker:
    ...
    @patch('spec_cli.ui.progress_utils.get_progress_manager')
    @patch('time.time')
    def test_progress_tracker_initialization(self, mock_time, mock_get_manager):
    ...
    @patch('spec_cli.ui.progress_utils.get_progress_manager')
    @patch('time.time')
    def test_progress_tracker_start_indeterminate(self, mock_time, mock_get_manager):
    ...
    @patch('spec_cli.ui.progress_utils.get_progress_manager')
    @patch('time.time')
    def test_progress_tracker_start_determinate(self, mock_time, mock_get_manager):
    ...
    @patch('spec_cli.ui.progress_utils.get_progress_manager')
    def test_progress_tracker_update_basic(self, mock_get_manager):
    ...
    @patch('spec_cli.ui.progress_utils.get_progress_manager')
    def test_progress_tracker_update_with_message(self, mock_get_manager):
    ...
    @patch('spec_cli.ui.progress_utils.get_progress_manager')
    def test_progress_tracker_auto_finish(self, mock_get_manager):
    ...
    @patch('spec_cli.ui.progress_utils.get_progress_manager')
    def test_progress_tracker_manual_finish(self, mock_get_manager):
    ...
    @patch('spec_cli.ui.progress_utils.get_progress_manager')
    @patch('time.time')
    def test_progress_tracker_get_statistics_basic(self, mock_time, mock_get_manager):
    ...
    @patch('spec_cli.ui.progress_utils.get_progress_manager')
    @patch('time.time')
    def test_progress_tracker_get_statistics_with_timing(self, mock_time, mock_get_manager):
    ...
    @patch('spec_cli.ui.progress_utils.get_progress_manager')
    @patch('time.time')
    def test_progress_tracker_context_manager(self, mock_time, mock_get_manager):
    ...
    @patch('spec_cli.ui.progress_utils.get_progress_manager')
    @patch('time.time')
    def test_progress_tracker_context_manager_exception(self, mock_time, mock_get_manager):
...
class TestConvenienceFunctions:
    ...
    def test_track_progress(self, mock_progress_tracker_class):
    ...
    def test_show_progress_for_files(self, mock_create_tracker):
    ...
    @patch('spec_cli.ui.progress_utils.create_file_progress_tracker')
    def test_show_progress_for_files_default_name(self, mock_create_tracker):
...
class TestProgressUtilsIntegration:
    ...
    @patch('spec_cli.ui.progress_utils.get_progress_manager')
    @patch('time.time')
    def test_complete_file_processing_workflow(self, mock_time, mock_get_manager):
    ...
    @patch('spec_cli.ui.progress_utils.get_progress_manager')
    @patch('time.time')
    def test_nested_progress_tracking(self, mock_time, mock_get_manager):
        ...
        with ProgressTracker("outer_operation", total_items=3, auto_finish=False) as outer_tracker:
            outer_tracker.update(1, "Starting phase 1")
            
            # Inner operation (disable auto-finish)
            with ProgressTracker("inner_operation", total_items=5, auto_finish=False) as inner_tracker:
                for i in range(5):
    ...
    @patch('spec_cli.ui.progress_utils.simple_progress')
    @patch('spec_cli.ui.progress_utils.spinner_context')
    def test_different_progress_types(self, mock_spinner_context, mock_simple_progress):
        ...
        with progress_context(total_items=10, description="Processing files") as update_func:
            update_func(5)
            mock_progress_bar.advance.assert_called_with(5)
    ...
    def test_time_formatting_comprehensive(self):
        ...
        for seconds, expected in test_cases:
            ...
            assert result == expected, f"Expected {expected} for {seconds}s, got {result}"

    def test_processing_speed_edge_cases(self):
    ...
    def test_operation_time_estimation_edge_cases(self):
