...
class TestTemplateConfig:
    ...
    def test_template_config_validates_required_placeholders(self):
    ...
    def test_template_config_rejects_empty_templates(self):
        """Test that empty templates are rejected."""
        with pytest.raises(ValidationError) as exc_info:
            TemplateConfig(index="", history="# {{filename}}")
        assert "should have at least" in str(exc_info.value) or "empty" in str(exc_info.value).lower()
    ...
    def test_template_config_validates_balanced_braces(self):
    ...
    def test_template_config_detects_malformed_placeholders(self):
        ...
        assert "malformed single braces" in str(exc_info.value) or "malformed placeholders" in str(exc_info.value)
    ...
    def test_template_config_provides_available_variables(self):
    ...
    def test_template_config_extracts_used_placeholders(self):
    ...
    def test_template_config_validates_ai_settings(self):
    ...
    def test_template_config_rejects_unknown_fields(self):
    ...
    def test_template_config_serializes_to_dict(self):
...
class TestTemplateValidator:
    ...
    def test_template_validator_validates_complete_config(self):
...
""",
    ...
    def test_template_validator_detects_missing_sections(self):
    ...
    def test_template_validator_validates_markdown_structure(self):
        """Test validation of markdown structure."""
        validator = TemplateValidator()
        
        # Templates not starting with headers
        config = TemplateConfig(
            index="No header {{filename}}",
            history="Also no header {{filename}}"
        )
        
        issues = validator.validate_config(config)
        assert any("should start with a markdown header" in issue for issue in issues)
    
    def test_template_validator_validates_ai_configuration(self):
        """Test AI configuration validation."""
        validator = TemplateValidator()
    ...
    def test_template_validator_provides_helpful_error_messages(self):
    ...
    def test_template_validator_raises_on_invalid_config(self):
