...
def _analyze_regex_for_singletons(
    ...
    singleton_patterns = [
        ...
        (r"@singleton_decorator", "decorator_usage"),
        (r"reset_singleton\(", "reset_function"),
...
def _is_singleton_class(node: ast.ClassDef) -> bool:
    ...
    # Check for @singleton_decorator
    for decorator in node.decorator_list:
        if isinstance(decorator, ast.Name) and decorator.id == "singleton_decorator":
            return True
...
def _is_singleton_import(module_name: str) -> bool:
    """Check if an import statement relates to singleton functionality."""
...
def _extract_singleton_name_from_line(line: str, pattern: str) -> str:
    ...
    elif "singleton_decorator" in line:
        return "singleton_decorator"
    elif "reset_singleton" in line:
        # Try to extract the class name from reset_singleton(ClassName)
        match = re.search(r"reset_singleton\(([^)]+)\)", line)
        if match:
            return match.group(1).strip()
        return "unknown_singleton"
    else:
        # Try to extract class name from class definition
        class_match = re.search(r"class\s+(\w*Singleton\w*)", line)
        if class_match:
